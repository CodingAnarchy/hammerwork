...
mod tests {
    ...
    async fn test_system_statistics() {
        ...
        let stats = collector.get_system_statistics(Duration::from_secs(60)).await.unwrap();
    ...
    async fn test_all_queue_statistics() {
        ...
        let all_stats = collector.get_all_statistics(Duration::from_secs(60)).await.unwrap();
        
        assert_eq!(all_stats.len(), 2);
        
        let email_stats = all_stats.iter().find(|s| s.queue_name == "email_queue").unwrap();
        ...
        let notification_stats = all_stats.iter().find(|s| s.queue_name == "notification_queue").unwrap();
    ...
    async fn test_cleanup_old_statistics() {
        ...
        let cleaned = collector.cleanup_old_statistics(Duration::from_secs(3600)).await.unwrap();
        assert_eq!(cleaned, 1);
