...
pub enum WorkerCommand {
    ...
    Start {
        ...
        queue: String,
        #[arg(short = 'c', long, default_value = "1", help = "Number of worker threads")]
        workers: u32,
...
async fn start_worker(
    ...
    info!("📊 Priority mode: {}", if strict_priority { "strict" } else { "weighted" });
...
async fn list_workers() -> Result<()> {
    ...
    table.set_header(vec![
        "Worker ID", "Queue", "Status", "Jobs Processed", "Uptime", "Last Seen"
    ]);
    
    // Mock data
    table.add_row(vec![
        "worker-001", "emails", "🟢 Running", "1,234", "2h 15m", "2s ago"
    ]);
    table.add_row(vec![
        "worker-002", "notifications", "🟡 Idle", "567", "1h 42m", "5s ago"
    ]);
    table.add_row(vec![
        "worker-003", "reports", "🔴 Error", "89", "45m", "2m ago"
    ]);
...
async fn show_worker_status(worker_id: Option<String>) -> Result<()> {
    if let Some(id) = worker_id {
        ...
        let mut table = comfy_table::Table::new();
        table.set_header(vec![
            "Worker ID", "Queue", "Status", "Uptime", "Jobs", "Success Rate", "Memory"
        ]);
        
        // Mock data
        table.add_row(vec![
            "worker-001", "emails", "🟢 Running", "2h 15m", "1,234", "98.5%", "45.2MB"
        ]);
        table.add_row(vec![
            "worker-002", "notifications", "🟡 Idle", "1h 42m", "567", "99.1%", "32.1MB"
        ]);
        table.add_row(vec![
            "worker-003", "reports", "🔴 Error", "45m", "89", "87.6%", "28.9MB"
        ]);
