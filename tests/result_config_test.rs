...
use hammerwork::{JobQueue, job::{Job, ResultStorage, ResultConfig}, queue::DatabaseQueue};
use serde_json::json;
...
async fn test_postgres_result_config_persistence() {
    ...
    assert_eq!(dequeued_job.result_config.ttl, Some(Duration::from_secs(3600)));
...
async fn test_mysql_result_config_persistence() {
    ...
    let job = Job::new("test_result_config_mysql".to_string(), json!({"task": "test"}))
        .with_result_config(config);
    ...
    // Dequeue the job to verify result_config is preserved
    let dequeued_job = queue.dequeue("test_result_config_mysql").await.unwrap().unwrap();
    
    assert_eq!(dequeued_job.id, job_id);
    assert_eq!(dequeued_job.result_config.storage, ResultStorage::Database);
    assert_eq!(dequeued_job.result_config.ttl, Some(Duration::from_secs(7200)));
